...
- **PRR@k and PRD@1 are implemented (Sprint 4); PCS and EVD are not yet.** FFR/SI/ERS/PRR@k/PRD@1 cover most of the proposal's intended metric set (`eiger/metrics/prr.py`, `eiger/metrics/prd.py`); PCS needs counterfactual re-generation support, and EVD requires the (not yet built) human-in-the-loop study. See [`docs/CLAIM_AND_RESEARCH_QUESTIONS.md` §6](CLAIM_AND_RESEARCH_QUESTIONS.md#6-metrics-proposed-vs-implemented).
