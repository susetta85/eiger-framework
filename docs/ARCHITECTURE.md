...
- **Three of the four built-in attacks (`numerical_shift`, `attribution_switch`, `date_manipulation`) can silently no-op, fixed in the Sprint 4 audit.** If a document has no digits/known-entity/year to modify, `apply()` still returns a `PoisonedDocument` with a full risk annotation and `doc_type="poisoned"`, byte-identical to the ground truth. Each attack now records `attack_params["no_op"] = True` in that case (no fallback text-injection was added, unlike `causal_manipulation`'s — inventing modified text when nothing matched would be a design change for `attribution_switch` in particular, not a bug fix). **Analyses computed before this fix cannot distinguish a no-op "poisoned" row from a genuinely poisoned one** — filter on `attack_params.get("no_op")` when re-analyzing older results, or re-run.
