...
| Poison-rate sweep experiment (0/1/3/5/10%) reproducing Phase 3 | Stable attacks + metrics above | 5 — **configs ready (Sprint 5+)**: `experiments/poison_rate_sweep/sweep_{0,1,3,5,10}pct.yaml` + `scripts/run_poison_rate_sweep.sh`; all 6 attacks applied uniformly at each rate, dense retriever, `ffr`/`source_integrity`/`ers`/`prr`/`prd`; execution and result interpretation still pending (requires live Qdrant + Ollama, run on a team member's machine, not yet executed) |
