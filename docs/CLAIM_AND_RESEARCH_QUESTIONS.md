...
| ~13.5% of the live Snopes claims file (`data/snopes/snopes_enriched.json`) contradicts the verified-true-only invariant | ✅ **Filter fixed** for future runs; ⚠️ **existing file not yet cleaned** — run `scripts/clean_snopes_contamination.py` |
| `numerical_shift`/`attribution_switch`/`date_manipulation` can silently no-op (return a "poisoned" document byte-identical to ground truth, still fully annotated) | ❌ Not yet fixed — needs a design decision on whether to mark `attack_params["no_op"]`, retry with a fallback (mirroring `causal`'s), or raise |
| `derive_seed()`'s collision-freedom claim doesn't hold if a `claim_id`/`attack.name` ever contains a colon (currently only exercised via `json_fixture` claim IDs, which are unvalidated user input) | ❌ Not yet fixed — needs a safer key construction (e.g. per-piece length-prefixing) |
| `ExperimentConfig.config_hash` is not truly order-independent for free-form `dict[str, Any]` fields (e.g. `AttackConfig.params`) — two semantically identical configs can hash differently | ❌ Not yet fixed — needs canonical (sorted-key) JSON serialization instead of `model_dump_json()`'s insertion-order default |
