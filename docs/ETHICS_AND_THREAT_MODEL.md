...
Section 6 (go/no-go checklist) is a **new engineering proposal written to operationalize the source protocol for this codebase** — it is not a transcription of a pre-existing team decision, and needs review/ratification by the whole team (not just the engineering side) before being treated as binding. Section 7 (`risk_level`/`sensitivity_class` fields) was a proposal in the same sense when this document was first written, but is now implemented in `eiger/core/models.py` as of Sprint 4 — see that section for exactly what "implemented" means here (the fields exist and validate; no dataset loader populates them yet).
...
## 7. `risk_level` / `sensitivity_class` fields (implemented Sprint 4)

`docs/CLAIM_AND_RESEARCH_QUESTIONS.md` §9 flagged adding `risk_level`/`sensitivity_class` fields to `Claim`/`Document` as a Sprint 4 task, dependent on this document existing to authoritatively define the fields' allowed values. Implemented as of Sprint 4:

- `Claim.sensitivity_class` / `Document.sensitivity_class`: typed, validated field — one of `"S0"`, `"S1"`, `"S2"`, `"S3"` (Section 5 above), or `None`. Invalid values raise a Pydantic `ValidationError` at construction time rather than being silently accepted as free-form metadata.
- `Claim.risk_level` / `Document.risk_level`: typed, validated integer field constrained to `1`-`5` (Section 5 above), or `None`.
- Both default to `None` rather than a guessed value when not explicitly classified — an unclassified item is never silently treated as S0/low-risk. **This convention is not enforced by the type system** (Pydantic cannot express "`None` means mandatory review, not safe"); any code that reads these fields (dataset loaders, the results matrix, any future publication-export tooling) must itself treat `None` as "requires classification before use," not as "safe by default."
- These live as first-class fields, not nested inside the pre-existing free-form `metadata: dict[str, Any]` field, specifically so they get real Pydantic validation (rejecting `"S4"` or `risk_level=7` at construction) rather than being silently accepted as arbitrary dict values.
- **Propagation:** `CorpusBuilder.build()` copies a claim's classification onto its ground-truth `Document`; each of the four built-in attacks (`numerical_shift`, `attribution_switch`, `date_manipulation`, `causal_manipulation`) copies the source document's classification onto the `PoisonedDocument` it returns — a poisoned variant of a classified item is not automatically less sensitive than its source. No dataset loader (`eiger.datasets`) sets these fields yet; they default to `None` for every claim loaded today, so this is infrastructure ready for classification data, not classification that has actually happened.
- **Follow-on fix — Qdrant/retriever round-trip:** both fields live on the base `Document` class, so any document passing through the default dense retriever + Qdrant needed the same round-trip fix already applied once this sprint for `PoisonedDocument` provenance (see `eiger/vector_stores/qdrant_store.py`'s `_document_to_payload()` and `eiger/retrieval/retriever.py`'s `_document_from_payload()`). `_document_to_payload()` now always includes `sensitivity_class`/`risk_level` in the base payload dict (not only for `PoisonedDocument`), and `_document_from_payload()` reads them back with `.get()` for both reconstruction branches, so a payload written before these fields existed still degrades to `None` rather than raising `KeyError`. Without this, a document's classification would have silently disappeared on retrieval — exactly the bug class the `RetrievedDocument.document` / `SerializeAsAny` fix and the original `PoisonedDocument` provenance fix both closed earlier this sprint. Covered by regression tests in `tests/unit/test_qdrant_store.py` and `tests/unit/test_retriever.py`.
