...
- **Only currently-implemented backends are supported**: ``retriever.type``
  must be "dense", "sparse", or "hybrid" (Sprint 4 — see
  ``eiger.retrieval.SparseRetriever``; Sprint 5 — see
  ``eiger.retrieval.HybridRetriever``), ``retriever.vector_store`` must be
  "qdrant", and ``llm.backend`` must be "ollama" — matching what actually
  exists in eiger.retrieval / eiger.vector_stores / eiger.llm today.
  Requesting any other value (e.g. "openai", which the config models already
  accept for forward-compatibility) raises ConfigurationError with an
  explicit, actionable message rather than failing confusingly deeper in
  the stack. Note that ``retriever.vector_store``/``retriever.embedder`` are
  still validated and used even for ``type="sparse"`` runs, because the
  embedder is also needed by ``EmbeddingFaithfulnessScorer`` — only the
