...
| `test_factcheck.py` | 25 | `FactCheckDataset` (implements `BaseDataset` directly — assumed JSONL format, see the class's own documented caveat): `"true"`-verdict-only filtering (case-insensitive), templated `context_query` fallback, direct reuse/prefixing of the raw `claim_id`, optional metadata passthrough (`verdict`/`article_url`/`date`), `split` file selection, `content_hash`, error paths (missing/invalid/non-object JSONL lines, missing `claim` field), and `download()`'s guard behavior |
